...
//"/beegfs/data/mmorel/catarina_gls/intermediate_domain/"
//path_file = "/beegfs/data/mmorel/Infer_Anth/align/"

params.sequences = path_file + "gymno_nooutgroup_domain.nt.fa"
//"intermediatedomain_GLS_nt.fasta"
//"GLS_cds.rename.fa"
//seed_plants
//"cds_only_catalytic_domain.fst"
//params.topology = path_file + "test_constrained_salse_nopara.nwk"
params.resdir= path_file +  "gymno/no_outgroup/results_gymno_aa_domain/" 
...
process trim_nonhomologous{
    ...
    -out_NT trim_GSI.nt.fasta -out_AA trim_GSI.aa.fasta
    '''  
    else if (hom_macse == "false")
    '''
    goalign translate --unaligned -i !{sequences} -o notrim_GSI.aa.fasta
    cp  !{sequences} notrim_GSI.nt.fasta
    '''
}
...
process align_macse{
    ...
    output:
    file 'macse.GSI_sequences.aa.fasta' into MACSE_Align
    shell:
    //aligns nucleotide (NT) coding sequences using their amino acid (AA) translations.
    '''
    java -jar ~/bin/macse_v2.07.jar -prog alignSequences -seq !{trim_sequences} -out_NT macse.GSI_sequences.fasta -out_AA macse.GSI_sequences.aa.fasta
    '''
}
...
process reconstruct_tree{
    ...
    '''
    iqtree -s !{align_aa} -m LG+R7+F -g !{topology} -nt !{task.cpus} -B 1000 -safe -bnni
    '''
}
