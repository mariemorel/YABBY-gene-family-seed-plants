...
params.resdir= path_file + "gymno/no_outgroup/results_gymno_codon_domain/" //do not forget to change the resdir
...
process align_macse{
    ...
    output:
    tuple file('macse.yabby_sequences.aa.fasta'), file('macse.yabby_sequences.fasta') into MACSE_Align
    shell:
    '''
...
process reconstruct_tree{
    ...
    if (guide_tree == "false")
    '''
    iqtree -s !{align_codon} -st CODON -m GY2K+G4 -nt !{task.cpus} -B 1000 -safe -bnni
    '''
    else if (guide_tree == "true")
    '''
    iqtree -s !{align_codon} -st CODON -g !{topology} -m GY2K+G4 -nt !{task.cpus} -B 1000 -safe -bnni
    '''
}
